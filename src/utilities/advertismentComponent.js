...
const AdvertisementComponent = ({ type }) => {
  ...
  useEffect(() => {
    fetch("https://api.sattakingvip.co.in/advertisementUpdate", {
      ...
      .then((response) => response.json())
      .then((json) => {
        // सिर्फ पहले 6 डेटा को लें
        const limitedData = json.slice(56, 60);
          ...
          {oddAdd.length > 0 ? (
            oddAdd.map((odd, index) => (
              <div key={index} className="col-md-12 ad-section">
                <div className="col-lg-12">
                  <h3>name: {odd?.name}</h3>
                  <h6 className="desc" dangerouslySetInnerHTML={{ __html: odd?.description }} />
                  <h4>Mobile: {odd?.mobile}</h4>
                  <h4>post: {odd?.post}</h4>
                  <div className="col-12">
                    <div className="row">
                      <div className="col-md-6 col-sm-12">
                        <a href={generateWhatsAppLink(odd.mobile, odd.name)} target="_blank" className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faWhatsapp} /> WhatsApp Now
                        </a>
                      </div>
                      <div className="col-md-6 col-sm-12">
                        <a href={generateCallLink(odd.mobile)} className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faPhone} /> Call Now
                        </a>
                      </div>
                    </div>
          ...
          {evenAdd.length > 0 ? (
            evenAdd.map((even, index) => (
              <div key={index} className="col-md-12 ad-section">
                <div className="col-lg-12">
                  <h3>name: {even?.name}</h3>
                  <h6 className="desc" dangerouslySetInnerHTML={{ __html: even?.description }} />
                  <h4>Mobile: {even?.mobile}</h4>
                  <h4>post: {even?.post}</h4>
                  <div className="col-12">
                    <div className="row">
                      <div className="col-md-6 col-sm-12">
                        <a href={generateWhatsAppLink(even.mobile, even.name)} target="_blank" className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faWhatsapp} /> WhatsApp Now
                        </a>
                      </div>
                      <div className="col-md-6 col-sm-12">
                        <a href={generateCallLink(even.mobile)} className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faPhone} /> Call Now
                        </a>
                      </div>
                    </div>
          ...
          {randomAdd.length > 0 ? (
            randomAdd.map((random, index) => (
              <div key={index} className="col-md-12 ad-section">
                <div className="col-lg-12">
                  <h3>name: {random?.name}</h3>
                  <h6 className="desc" dangerouslySetInnerHTML={{ __html: random?.description }} />
                  <h4>Mobile: {random?.mobile}</h4>
                  <h4>post: {random?.post}</h4>
                  <div className="col-12">
                    <div className="row">
                      <div className="col-md-6 col-sm-12">
                        <a href={generateWhatsAppLink(random.mobile, random.name)} target="_blank" className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faWhatsapp} /> WhatsApp Now
                        </a>
                      </div>
                      <div className="col-md-6 col-sm-12">
                        <a href={generateCallLink(random.mobile)} className="form-control m-2 btn ad-btn">
                          <FontAwesomeIcon icon={faPhone} /> Call Now
                        </a>
                      </div>
                    </div>
